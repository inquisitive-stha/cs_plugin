<?php
template_start('Label Settings', 'Add New', 'javascript: doAddNew();');
?>
<table class="wp-list-table widefat fixed striped pages">
    ...
    <tbody id="nrow">
        ...
        <tr id="tr-<?php echo $l->id; ?>">
            <th>
                <input type="text" disabled class="db_l_key form-control percent_50" value="<?php echo $l->l_key; ?>" />
            </th>
            <th>                
                <input type="text" disabled class="db_l_value form-control percent_50" value="<?php echo $l->l_value; ?>" />
        ...
        <tr id="add_tmpl" style="display: none;">
            <th>                
                <input type="text" class="db_l_key form-control percent_50" value="" />
            </th>
            <th>                
                <input type="text" class="db_l_value form-control percent_50" value="" />
            </th>
            <th>
...
<script type="text/javascript">
    ...
    function doEdit(b)
    {
         ...
         if(jQuery(b).html() == 'Edit')
        {
            ...
            jQuery(tr).find('.db_l_key').removeAttr('disabled');
            jQuery(tr).find('.db_l_value').removeAttr('disabled');
            jQuery(b).html('Save');
            return;
        }     
        
        l_key = jQuery(tr).find('.db_l_key').val();
        l_value = jQuery(tr).find('.db_l_value').val();
        id = jQuery(tr).find('.db_id').val();
        
        jQuery(tr).find('input[type="text"]').hide();
        jQuery(tr).find('.db_l_key').after('<span class="loading">Processing...</span>');
        ...
        jQuery.ajax({
                ...
                data: { action: "savelabel", l_key: l_key, l_value:l_value, id: id}
              })
                .done(function( msg ) {
                    if(msg.success == '1')
                    {
    ...
    function doAdd(b)
    {
        ...
        jQuery.ajax({
                ...
                dataType: "json",
                url: "<?php echo admin_url('admin-ajax.php'); ?>",
                data: { action: "savelabel", l_key: l_key,l_value:l_value}
              })
                .done(function( msg ) {
                    if(msg.success == '1')
                    {
